...
public class DemoService {
	...
	private void createTaskStages() {
		String[] taskStages = { "To Do", "Doing", "Code Review", "Testing",
				"Done" };
	...
	private void createProjects() {
		...
		Project project = new Project("Demo project 1",
				"Demo project 1 description");
	...
	private void createIterations() {
		...
		Iteration iteration = new Iteration("First iteration",
				"Iteration description", projects.get(0));
		iteration.setStartDate(start.toDate());
		iteration.setEndDate(end.toDate());
		iterationService.save(iteration);

		iteration = new Iteration("Second iteration", "Iteration description",
				projects.get(0));
		...
		iteration = new Iteration("Third iteration", "Iteration description",
				projects.get(0));
		start = start.plusWeeks(2);
		end = end.plusWeeks(2);
		iteration.setStartDate(start.toDate());
		...
		iteration = new Iteration("Iteration 1", "Iteration description",
				projects.get(1));
		start = start.plusWeeks(2);
		end = end.plusWeeks(2);
		iteration.setStartDate(start.toDate());
	...
	private void createTasks() {
		...
		List<Iteration> iterations = iterationService.getList(projects.get(0)
				.getId());
		...
		for (int i = 0; i < 40; i++) {
			Task task = new Task("Task " + random.nextInt(10),
					"Task description " + random.nextInt(10), types.get(random
							.nextInt(3)), projects.get(random.nextInt(2)));
			...
			taskService.addComment(task, new Comment("Just random comment "
					+ random.nextInt(100), task));
			taskService.addComment(task, new Comment("Another random comment "
					+ random.nextInt(100), task));
		}
	}
}
